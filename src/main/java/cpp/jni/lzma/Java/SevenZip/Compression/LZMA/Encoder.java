package cpp.jni.lzma.Java.SevenZip.Compression.LZMA;

import cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.BitTreeEncoder;
import cpp.jni.lzma.Java.SevenZip.Compression.LZMA.Base;
import cpp.jni.lzma.Java.SevenZip.Compression.LZ.BinTree;
import cpp.jni.lzma.Java.SevenZip.ICodeProgress;
...
public class Encoder
{
	...
	class LiteralEncoder
	{
		class Encoder2
		{
			...
			public void Init() { cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.InitBitModels(m_Encoders); }



			public void Encode(cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder rangeEncoder, byte symbol) throws IOException
			{
				int context = 1;
				for (int i = 7; i >= 0; i--)
				{
			...
			public void EncodeMatched(cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder rangeEncoder, byte matchByte, byte symbol) throws IOException
			{
				int context = 1;
				boolean same = true;
				for (int i = 7; i >= 0; i--)
			...
			public int GetPrice(boolean matchMode, byte matchByte, byte symbol)
			{
				...
				if (matchMode)
				{
					for (; i >= 0; i--)
					{
						int matchBit = (matchByte >> i) & 1;
						int bit = (symbol >> i) & 1;
						price += cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.GetPrice(m_Encoders[((1 + matchBit) << 8) + context], bit);
						context = (context << 1) | bit;
						if (matchBit != bit)
						{
							i--;
				...
				{
					int bit = (symbol >> i) & 1;
					price += cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.GetPrice(m_Encoders[context], bit);
					context = (context << 1) | bit;
				}
				return price;
			}
	...
	class LenEncoder
	{
		...
		{
			cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.InitBitModels(_choice);
		...
		public void Encode(cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder rangeEncoder, int symbol, int posState) throws IOException
		{
		...
		{
			int a0 = cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_choice[0]);
			int a1 = cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_choice[0]);
			int b0 = a1 + cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_choice[1]);
			int b1 = a1 + cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_choice[1]);
			int i = 0;
	...
	cpp.jni.lzma.Java.SevenZip.Compression.LZ.BinTree _matchFinder = null;
	cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder _rangeEncoder = new cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder();
	...
	void Create()
	{
		...
		{
			cpp.jni.lzma.Java.SevenZip.Compression.LZ.BinTree bt = new cpp.jni.lzma.Java.SevenZip.Compression.LZ.BinTree();
			int numHashBytes = 4;
	...
	void Init()
	{
		...
		cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.InitBitModels(_isMatch);
		cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.InitBitModels(_isRep0Long);
		cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.InitBitModels(_isRep);
		cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.InitBitModels(_isRepG0);
		cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.InitBitModels(_isRepG1);
		cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.InitBitModels(_isRepG2);
		cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.InitBitModels(_posEncoders);
	...
	{
		return cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isRepG0[state]) +
				cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isRep0Long[(state << Base.kNumPosStatesBitsMax) + posState]);
	}
	...
	int GetPureRepPrice(int repIndex, int state, int posState)
	{
		...
		if (repIndex == 0)
		{
			price = cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isRepG0[state]);
			price += cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isRep0Long[(state << Base.kNumPosStatesBitsMax) + posState]);
		}
		else
		{
			price = cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isRepG0[state]);
			if (repIndex == 1)
				price += cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isRepG1[state]);
			else
			{
				price += cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isRepG1[state]);
				price += cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.GetPrice(_isRepG2[state], repIndex - 2);
			}
		}
		return price;
	}
	...
	int GetOptimum(int position) throws IOException
	{
		...
		_optimum[1].Price = cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isMatch[(_state << Base.kNumPosStatesBitsMax) + posState]) +
				_literalEncoder.GetSubCoder(position, _previousByte).GetPrice(!Base.StateIsCharState(_state), matchByte, currentByte);
		_optimum[1].MakeAsChar();

		int matchPrice = cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isMatch[(_state << Base.kNumPosStatesBitsMax) + posState]);
		int repMatchPrice = matchPrice + cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isRep[_state]);
		...
		int normalMatchPrice = matchPrice + cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isRep[_state]);
		...
		while (true)
		{
			...
			int curAnd1Price = curPrice +
					cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isMatch[(state << Base.kNumPosStatesBitsMax) + posState]) +
			...
			matchPrice = curPrice + cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isMatch[(state << Base.kNumPosStatesBitsMax) + posState]);
			repMatchPrice = matchPrice + cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isRep[state]);
			...
			if (!nextIsChar && matchByte != currentByte)
			{
				...
				if (lenTest2 >= 2)
				{
					...
					int nextRepMatchPrice = curAnd1Price +
							cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]) +
							cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isRep[state2]);
					{
					...
					if (lenTest2 >= 2)
					{
						...
						int curAndLenCharPrice =
								repMatchPrice + GetRepPrice(repIndex, lenTest, state, posState) +
										cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]) +
						...
						int nextMatchPrice = curAndLenCharPrice + cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]);
						int nextRepMatchPrice = nextMatchPrice + cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isRep[state2]);
			...
			if (newLen >= startLen)
			{
				normalMatchPrice = matchPrice + cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isRep[state]);
	...
	void FillDistancesPrices()
	{
		...
		for (int lenToPosState = 0; lenToPosState < Base.kNumLenToPosStates; lenToPosState++)
		{
			...
			for (posSlot = Base.kEndPosModelIndex; posSlot < _distTableSize; posSlot++)
				_posSlotPrices[st + posSlot] += ((((posSlot >> 1) - 1) - Base.kNumAlignBits) << cpp.jni.lzma.Java.SevenZip.Compression.RangeCoder.Encoder.kNumBitPriceShiftBits);
